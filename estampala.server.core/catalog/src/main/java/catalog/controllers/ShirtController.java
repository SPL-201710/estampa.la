...
public class ShirtController extends EstampalaController {
	...
	public ResponseEntity<Page<Shirt>> getAll(@PathVariable int page, @PathVariable int pageSize) {		
	...
	public ResponseEntity<Shirt> create(@RequestBody Shirt element) throws ShirtAlreadyExistsException {		
		if(service.exists(element.getId())) {
			throw new ShirtAlreadyExistsException();
		}
				
		return new ResponseEntity<Shirt>(element, HttpStatus.OK);
	}
